...
SETUP_EXISTING_SETTINGS() {
	PRINT_MESSAGE "Existing Config"
	USERNAME=$(sudo grep 'Username ' /etc/cntlm.conf | awk '{print $2}')
	DOMAIN=$(sudo grep 'Domain ' /etc/cntlm.conf | awk '{print $2}')
	EXISTING_PROXY=$(sudo grep -v '^[#|No]' /etc/cntlm.conf | grep "Proxy" | awk '{print $2}')
	PROXY_IP=${EXISTING_PROXY%:*}
	PROXY_PORT=${EXISTING_PROXY##*:}
}
...
EXISTING_CONFIG_CHECK=$(sudo grep 'Domain		corp-uk' /etc/cntlm.conf)

if [ -n "${EXISTING_CONFIG_CHECK}"]; then
	SETUP_EXISTING_SETTINGS
	# Display back Domain Creds
	HIGHLIGHT
	PRINT_GOOD "Username - $USERNAME"
